        ...
        if (attributes && attributes.length) {
          ...
          if (seal) {
            ...
            const getTypeName = (typeName) => {
              return build_map[typeName] || getGeneric(typeName) || `I${typeName}`;
            }
            main: for (const property of properties) {
              ...
              if (nodeType === 44) {
                ...
                switch (nodeType) {
                  case 6: {
                    const type = getTypeName(typeAnnotation.name);
                    pros.push({
                      identifier: id.name,
                      type,
                      isArray: false,
                  ...
                  case 117: {
                    ...
                    const typeName = typeArguments[0].name;
                    const type = getTypeName(typeName);
                    switch (pre) {
                      case 'Vec':
                      case 'HashSet':
                        pros.push({
